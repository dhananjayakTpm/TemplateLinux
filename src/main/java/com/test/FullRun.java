...
/**
 * Implements API automation with jira Xray integartion with bug reporting
 * 
 * @author nasia.t
 *
 */
public class FullRun {
	...
	private static XrayAPIIntegration apiIntegration = new XrayAPIIntegration();
	private static CreateIssueDTO createIssueDTO = null;
	private CreateIssueDTO createBugDTO = null;
	...
	public void afterAllTest() throws URISyntaxException {
		List<TestCase> testCaseList = XrayAPIIntegration.getTestExecution(testExecutionid);
		List<BugDTO> jasperBugDTOList = new ArrayList<>();
		try {

			testCaseList.forEach(a -> {
				String bugDescription = "AtoBe bug description for ";
				BugDTO jasperBugDTO = new BugDTO();
				if (a.getStatus().equalsIgnoreCase("FAIL")) {
					...
					if (list != null) {
						...
						if (list != null) {
							list.getIssues().stream()
									.filter(b -> b.getFields().getDescription().equals(bugDescription + a.getKey()))
									.forEach(e -> {
										Issue issue = apiIntegration.getIssue(e.getKey());
										if (issue.getFields().getStatus().getName().equalsIgnoreCase("CLOSED")
												|| issue.getFields().getStatus().getName().equalsIgnoreCase("DONE")
												|| issue.getFields().getStatus().getName()
														.equalsIgnoreCase("RESOLVED")) {
											TransitionList transitionList = apiIntegration
													.getTransitionsFromBug(issue.getKey());
											transitionList.getTransitions().stream()
													.filter(c -> c.getName().equalsIgnoreCase("Reopened"))
													.forEach(d -> apiIntegration.postTransitions(issue.getKey(),
															d.getId(), "Bug has been Reopened for " + testExecutionid));
										} else {
											TransitionList transitionList = apiIntegration
													.getTransitionsFromBug(issue.getKey());
											transitionList.getTransitions().stream()
													.filter(c -> c.getName()
															.equalsIgnoreCase(issue.getFields().getStatus().getName()))
													.forEach(d -> apiIntegration.postTransitions(issue.getKey(),
															d.getId(),
															"Bug reported in "
																	+ issue.getFields().getStatus().getName()
																			.toUpperCase()
																	+ " status for " + testExecutionid));
										}
										jasperBugDTO.setLinkedBugId(e.getKey());
										jasperBugDTO.setBugLink(BASE_URL + BROWSE + e.getKey());
									});
							if (list.getIssues().stream()
									.filter(b -> b.getFields().getDescription().equals(bugDescription + a.getKey()))
									.count() <= 0) {
								createBugDTO = new CreateIssueDTO();
								createBugDTO.setDescription(bugDescription + a.getKey());
		...
		try {
			TestRun response = apiIntegration.getTestRun(testCaseList.get(0).getKey(), testExecutionid);
			...
			jasperReportDTO.setStartedDate(testCaseList.get(0).getStartedOn());
			jasperReportDTO.setEndDate(testCaseList.get(43).getFinishedOn());
			jasperReportDTO.setJasperBugDTO(jasperBugDTOList);
			jasperReportDTO.setAssignee("assignee");
			jasperReportDTO.setExecutedBy("ThinkPalm");
			...
			int failCount = (int) jasperBugDTOList.stream().filter(a -> a.getTestStatus().equalsIgnoreCase("FAIL"))
					.count();
			int bugCount = (int) jasperBugDTOList.stream().filter(a -> a.getTestStatus().equalsIgnoreCase("FAIL"))
					.count();
			List<BugDTO> uc1List = jasperBugDTOList.subList(0,14);
			List<BugDTO> uc2List = jasperBugDTOList.subList(14, 36);
			List<BugDTO> uc3List = jasperBugDTOList.subList(36, 39);
			List<BugDTO> uc4List = jasperBugDTOList.subList(39, 44);

			Date startedDateTime = jasperReportDTO.getStartedDate();
			Instant instant = startedDateTime.toInstant();
			LocalDateTime localDateTime = instant.atZone(ZoneId.systemDefault()).toLocalDateTime();
			DateTimeFormatter myFormatObj = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss");
			String formattedDate = localDateTime.format(myFormatObj);

			Date finishedDateTime = jasperReportDTO.getEndDate();
			...
			test1.sendEmailWithTemplate("A-to-Be Xray Test Execution Report-FULL RUN", Arrays.asList(MAIL_TO),
